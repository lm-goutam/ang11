...
export class IntgUpdateComponent implements OnInit{
  [x: string]: any;
   ...
   getOrg(){
    ...
    onIntgsUpdate(integration: {app_url: string,comment: string,i_app:string,i_cms: number,i_status: number}){

      var reqObj = integration;
      reqObj.i_app = JSON.stringify(integration.i_app);
  
      this.OrgData.updateIntgsById(this.intgData.I_id,reqObj).subscribe((integration)=>{
